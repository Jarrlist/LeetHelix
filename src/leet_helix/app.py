...
from .challenges import (load_challenges, select_smart_challenge, get_milestone,
    build_file_content
)
from .cfg import app, console, REPO_URL
...
def play(challenge_id: str = typer.Argument(None, help="The ID of the challenge to play")):
    ...
    if not formatted_goals:
        console.print("[red]Goal files exist but could not be read![/red]")
        return
...
def upgrade_package(force: bool = False):
    ...
    try:
        ...
        console.print("[bold green]Successfully upgraded![/bold green]")
